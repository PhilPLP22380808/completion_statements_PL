...
export default function App() {
  ...
  const generatePDF = () => {
    ...
    y += 8;
    addText('This completion statement was generated by Pinnacle Property Lawyers.', pageWidth / 2, y, { size: 8, color: lightGray });
...
export default function App() {
  ...
  const handlePurchasePriceChange = (e) => {
    const raw = e.target.value.replace(/,/g, '');
    setPurchasePrice(raw);
    setPurchasePriceDisplay(formatWithCommas(raw));
  };
  ...
  const handleAllowanceAmountChange = (id, value) => {
    const raw = value.replace(/,/g, '');
    setAllowances(allowances.map(a =>
      a.id === id ? { ...a, amount: raw, amountDisplay: formatWithCommas(raw) } : a
    ));
  };
  ...
  const handleApportionmentMoneyChange = (id, field, value) => {
    ...
    const displayField = field + 'Display';
    setApportionments(apportionments.map(a =>
      a.id === id ? { ...a, [field]: raw, [displayField]: formatWithCommas(raw) } : a
    ));
  };
